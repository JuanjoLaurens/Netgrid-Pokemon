<template>
  <div class="liked-pokemons">
    <h4 class="mb-5">Pokemones que no te gustan</h4>
    <div class="container">
    <table class="table table-hover liked-table shadow-lg">
      ...
      <tbody>
        <tr class="text-center" v-for="(pokemon, index) in this.likedPokemons" :key="index">
          ...
          <td><button class="btn btn-info" data-bs-toggle="modal" :data-bs-target="`#listdetailsModal-${pokemon.pokemon_id}`">Más detalles</button></td>
...
export default {
  ...
  methods:{
     ...
     fetchData() {
        ...
        .then((result)=>{
          ...
          console.log("Gusta: ", this.likedPokemons);
        })
        .catch((error) => {
            console.log("El problema ", error);
        });
    },
  ...
  mounted(){
    console.log(this.capitalize("Juan"));
